...
class Game:  # The game itself
    ...
    def step(self):
        ...
        x_history.append(self.ball.x_bin)
        y_history.append(self.ball.y_bin)
        # x_history.append(delta_x)
        # y_history.append(delta_y)
        feature_history.append(self.label.argmax())
...
def running_mean(array, weights):
    res = []
    for i in range(len(array) - T):
        r = np.average(array[i: i + T], weights=weights)
        res.append(r)
    return res
...
if __name__ == '__main__':
    ...
    # x_history = np.array_split(x_history, T)
    # y_history = np.array_split(y_history, T)
    # feature_history = np.array_split(feature_history, T)

    w = np.arange(T) / T
    x_history = running_mean(x_history, w)
    y_history = running_mean(y_history, w)
    feature_history = running_mean(feature_history, w)

    time_stamps = np.arange(len(x_history))
    plt.subplot(321)
    plt.hist(x_history)
    plt.subplot(322)
    plt.plot(time_stamps, x_history)
    plt.subplot(323)
    plt.hist(y_history)
    plt.subplot(324)
    plt.plot(time_stamps, y_history)
    plt.subplot(325)
    plt.hist(feature_history)
    plt.subplot(326)
    plt.plot(time_stamps, feature_history)
    plt.show()
